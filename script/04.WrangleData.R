...
library(QPAD) #offset calculation
library(terra) #offset calculation
library(intrval) #offset calculation
...
complete <- use |>
  ...
  dplyr::filter(gisid %in% d.long.pts$UID,
                gisid %in% row.names(d.wide.150$veg.current),
                gisid %in% row.names(d.wide.564$veg.current))

#CALCULATE OFFSETS####

#TO DO: CALCULATE OFFSETS FOR SPECIES WITHOUT QPAD####
...
spp.use <- complete |>
  ...
  pivot_longer(ALFL:YTVI, names_to="species", values_to="detection") |>
  group_by(species) |>
...
spp <- sort(intersect(getBAMspecieslist(), colnames(bird)))
...
#6. Put together----
offsets <- data.frame(surveyid = complete.local$surveyid) |>
...
#Veg
tv <- read.csv(file.path(root, "Data", "lookups", "lookup-veg-hf-age-v2024.csv"))
row.names(tv) <- tv$ID

#Soil
ts <- read.csv(file.path(root, "Data", "lookups", "lookup-soil-hf-v2024.csv"))
row.names(ts) <- ts$ID
...
covs1 <- complete |>
  ...
  left_join(d.long.pts |>
              rename(gisid = UID) |>
              dplyr::select(gisid, NRNAME, NSRNAME)) |>
  left_join(climate.data |>
              rename(gisid = UID) |>
              dplyr::select(gisid, paspen, HTV, MAT, FFP, TD, MAP, MSP, PAS, CMD, AHM, SHM, Eref))
...
vc2 <- data.frame(d.wide.150$veg.current) |>
  mutate_all(~./999291) |>
  rownames_to_column(var = "gisid") |>
  inner_join(covs1 |> dplyr::select(gisid, surveyid), multiple="all") |>
  column_to_rownames("surveyid") |>
...
#10. Add nuisance variables, modifier variables, and tidy----
#Surveys that are BBS or surveys with > 0.4 proportion "is_road"
#Fix factor levels for vegc and soilc
...
covs <- covs7 |>
  dplyr::select(surveyid, gisid, X, Y, vegc, soilc, wtAge, wtAge2, wtAge05, isCon, isUpCon, isBogFen, isMix, isPine, isWSpruce, fcc2, road, EnSoftLin, HardLin, TrSoftLin, EnSeismic, Wellsites, Soft, method, pWater_KM, pWater2_KM, HTV:Eref)

bird <- complete |>
  dplyr::select(surveyid, gisid, all_of(spp)) |>
